...
import type { IBSplineR1toR2 } from "./IBSplineR1toR2"
...
export abstract class BaseBSplineR1toR2 implements IBSplineR1toR2 {
     ...
     */
    protected abstract factory(controlPoints: readonly Vector2d[], knots: readonly number[]): this
    ...
    abstract get freeControlPoints(): readonly Vector2d[] 
    ...
    evaluate(u: number) : Vector2d {
        ...
        for (let i = 0; i < this._degree + 1; i += 1) {
            x += basis[i] * this._controlPoints[span - this._degree + i].x
            y += basis[i] * this._controlPoints[span - this._degree + i].y
        }
